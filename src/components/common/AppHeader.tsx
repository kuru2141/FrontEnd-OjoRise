...
function AppHeader() {
    ...
    const showKakaoLoginButton = !isLoggedIn && !isGuest;
    const showMypageMenu = isLoggedIn || isGuest;
    const showPlans = true; // 항상 표시
        ...
        >
            ...
            >
                ...
                <div className="flex items-center space-x-[50px]">
                    {/* 요금제 둘러보기는 항상 표시 */}
                    {showPlans && (
                        <Link href="/" className="text-sm text-neutral-800 hover:text-gray-600 transition-colors duration-300">
                            요금제 둘러보기
                        </Link>
                    )}

                    {/* 마이페이지/로그아웃 메뉴 (로그인 또는 게스트 상태일 때만 표시) */}
                    {showMypageMenu && ( // 조건부 렌더링으로 변경
                        <>
                            <Link href="/" className="text-sm text-neutral-800 hover:text-gray-600 transition-colors duration-300">
                                마이페이지
                            </Link>
